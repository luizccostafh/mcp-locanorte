...
----------------------------------------------------------------------------
 ...
 Tools:
   ...
   - fluxo_caixa        -> títulos em aberto por janela de vencimento
                           (vencido/7/15/30/30+), posição por conta e
                           projeção líquida 7/15/30 dias.
...
----------------------------------------------------------------------------
 v1.5.0 — (1) FIX: `_fetch_csv` agora usa follow_redirects=True — o hub
   Kondado responde 302 -> URL pré-assinada do S3; sem seguir o redirect a
   resposta vinha vazia (raise_for_status não acusa 3xx).
   (2) Nova tool `fluxo_caixa`: buckets por vencimento, posição líquida por
   conta corrente e projeção líquida (a receber − a pagar) em 7/15/30 dias.
   Colunas de vencimento/conta são env-overridáveis (KONDADO_COL_VENCIMENTO,
   KONDADO_COL_CONTA) — se vierem em branco, o bloco degrada sem quebrar.
...
# --- Colunas de fluxo de caixa (env-overridáveis; ajuste no Render se diferir) ---
COL_VENC  = os.environ.get("KONDADO_COL_VENCIMENTO", "data_vencimento")
COL_CONTA = os.environ.get("KONDADO_COL_CONTA",      "conta_corrente")
...
def _fetch_csv(tabela: str) -> list[dict[str, str]]:
    ...
    # O hub Kondado responde 302 -> URL pré-assinada do S3; é preciso SEGUIR o redirect.
...
def _parse_data(data_str: str | None) -> date | None:
    """Converte data ISO (YYYY-MM-DD...) ou BR (DD/MM/YYYY) em date. None se falhar."""
    s = str(data_str or "").strip()
    if len(s) >= 10 and s[4] == "-" and s[7] == "-":       # ISO
        try:
...
def _dias_ate(data_str: str | None) -> int | None:
    """Dias entre hoje (America/Sao_Paulo) e a data informada. None se não reconhecer."""
...
# ----------------------------------------------------------------------------
# 4.3) SUB-BLOCOS DE FLUXO DE CAIXA
# ----------------------------------------------------------------------------
def _fluxo_de_titulos(tabela: str, abertos: set[str], cancelados: set[str]) -> dict[str, Any]:
    """
    Para UMA tabela de títulos (pagar OU receber), considerando só os EM ABERTO:
      - total_em_aberto : soma de valor_documento dos títulos em aberto
      - por_janela      : buckets por data_vencimento
                          (vencido / ate_7 / ate_15 / ate_30 / acima_30 / sem_vencimento)
      - acumulado_ate   : soma cumulativa de quem vence até 7/15/30 dias (inclui vencidos)
      - por_conta       : soma por conta corrente (poucas contas → não trunca)
    """
    rows = _fetch_csv(tabela)
    buckets = {"vencido": 0.0, "ate_7": 0.0, "ate_15": 0.0, "ate_30": 0.0,
               "acima_30": 0.0, "sem_vencimento": 0.0}
    qtd = {k: 0 for k in buckets}
    acum = {7: 0.0, 15: 0.0, 30: 0.0}
    por_conta: dict[str, float] = {}
    total_aberto = 0.0

    for r in rows:
        st = str(r.get(COL_STATUS, "") or "").strip().upper()
        if st in cancelados or st not in abertos:
            continue
        v = _to_float(r.get(COL_VALOR_DOC))
        total_aberto += v

        conta = str(r.get(COL_CONTA, "") or "").strip() or "(sem conta)"
        por_conta[conta] = por_conta.get(conta, 0.0) + v

        dias = _dias_ate(r.get(COL_VENC))
        if dias is None:
            buckets["sem_vencimento"] += v
            qtd["sem_vencimento"] += 1
            continue
        if dias < 0:
            b = "vencido"
        elif dias <= 7:
    ...
    return {
        ...
        "por_janela": {k: {"valor": round(buckets[k], 2), "qtd": qtd[k]} for k in buckets},
        "acumulado_ate": {f"{w}d": round(acum[w], 2) for w in (7, 15, 30)},
        "por_conta": {c: round(val, 2)
                      for c, val in sorted(por_conta.items(), key=lambda x: x[1], reverse=True)},
    }


def _bloco_fluxo_caixa() -> dict[str, Any]:
    """
    Fluxo de caixa a partir dos títulos em aberto (pagar + receber):
      - contas_a_pagar / contas_a_receber : buckets, acumulado e por conta
      - posicao_por_conta                 : (a receber − a pagar) em aberto, por conta
      - projecao_liquida                  : (receber − pagar) acumulada até 7/15/30 dias
    Cada sub-bloco é protegido isoladamente (degradação graciosa).
    """
    if not KONDADO_TOKEN:
        return {
            "status": "indisponivel",
    ...
    pagar   = _safe("fluxo_pagar",   lambda: _fluxo_de_titulos(TBL_PAGAR, abertos, cancelados))
    receber = _safe("fluxo_receber", lambda: _fluxo_de_titulos(TBL_RECEBER, abertos, cancelados))

    projecao: dict[str, Any] = {}
    posicao_por_conta: dict[str, float] = {}
    if pagar.get("status") == "ok" and receber.get("status") == "ok":
        for w in ("7d", "15d", "30d"):
            r = receber["acumulado_ate"].get(w, 0.0)
            p = pagar["acumulado_ate"].get(w, 0.0)
            projecao[w] = round(r - p, 2)
        contas = set(pagar["por_conta"]) | set(receber["por_conta"])
        bruto = {c: round(receber["por_conta"].get(c, 0.0) - pagar["por_conta"].get(c, 0.0), 2)
                 for c in contas}
        posicao_por_conta = dict(sorted(bruto.items(), key=lambda x: x[1], reverse=True))
    else:
        projecao = {"status": "indisponivel", "obs": "depende de pagar e receber OK"}

    return {
        "status": "ok",
        "fonte": KONDADO_BASE_URL,
        "criterio": ("Em aberto = status em {A VENCER, ATRASADO, VENCE HOJE}; janelas por "
                     f"'{COL_VENC}'; conta por '{COL_CONTA}'; projeção líquida (receber − pagar) "
                     "acumulada até 7/15/30 dias (inclui vencidos)."),
        "contas_a_pagar": pagar,
        "contas_a_receber": receber,
        "posicao_por_conta": posicao_por_conta,
        "projecao_liquida": projecao,
    }
...
def fluxo_caixa() -> dict:
    """Fluxo de caixa da Locanorte (JSON): títulos em aberto por janela de vencimento
    (vencido/7/15/30/30+), posição líquida por conta corrente e projeção líquida 7/15/30 dias."""
    return {
        "contrato_versao": CONTRATO_VERSAO,
        "data_referencia": datetime.now(TZ).isoformat(),
        "fluxo_caixa": _bloco_fluxo_caixa(),
    }
