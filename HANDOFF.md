...
> **Atualizado em 2026-06-17** para refletir o código realmente no ar (`server.py` v1.8.0).
...
Claude → Settings → Connectors → conector "MCP Locanorte"
        ▼
Tools: status_locanorte, resumo_locanorte, faturamento, fluxo_caixa, dre_resultado, top_clientes (dados reais)
```
...
## 3. server.py atual (v1.8.0 — no ar)
...
- Helpers: `_to_float` (BR/EN com sinal), `_norm_cod`, `_competencia_de`, `_safe` (degradação
  ...
  **(v1.6.0)** `_valida_competencia` (normaliza/valida 'AAAA-MM' e 'AAAA/MM', erro amigável) —
  centraliza a regra para `resumo_locanorte`, `faturamento` e `dre_resultado`.
- Sub-blocos de negócio: `_resumir_titulos` (pagar/receber + top categorias),
  ...
  **(v1.7.0)** `_dre_resultado` (soma de `valor` já com sinal, separando realizado/projetado pela data).
  **(v1.8.0)** `_top_clientes` (ranking por contas a receber) + `_detecta_coluna` (acha a coluna de
  cliente no header sem chutar; auto-detecção com fallback que lista as colunas disponíveis).
- Env vars novas **(v1.5.0)**: `KONDADO_COL_VENCIMENTO` (default `data_vencimento`) +
  ...
  **(v1.7.0)** `KONDADO_COL_DRE_N1` (default `descricaodre_n1`) para a quebra por linha do DRE.
- Tools expostas:
  - `status_locanorte()` → dict (health-check estruturado).
  - `resumo_locanorte(competencia=None)` → dict (cadastro + financeiro ao vivo). **(v1.6.0)**
    `competencia` ('AAAA-MM') reposiciona só o faturamento_mes; títulos seguem a posição ATUAL.
  - `faturamento(competencia=None)` → dict **(v1.6.0)**: receita de um mês isolado (busca só
    `tabela_dre_omie`): receita_bruta, deducoes e receita_liquida.
  ...
  - `dre_resultado(ano=None, competencia=None)` → dict **(v1.7.0)**: Resultado Operacional do DRE
    separando REALIZADO (competências ≤ mês corrente) de PROJETADO (futuras), derivado da data
    (confirmado via .pbix que não há coluna "Tipo Período"). Quebra por mês e por `descricaodre_n1`.
    Validado contra o doc DAX: Realizado +629.043,78 / Projetado −780.816,31 / Total −151.772,53.
  - `top_clientes(limite=10)` → dict **(v1.8.0)**: ranking de clientes por contas a receber
    (`valor_total` + `valor_em_aberto`). Coluna de cliente auto-detectada (env `KONDADO_COL_RECEBER_CLIENTE`
    + candidatos); se nenhuma bater, retorna `indisponivel` com `colunas_disponiveis`. Validar ao vivo.
...
2. **Tools operacionais** — ✅ `fluxo_caixa` (v1.5.0) entregue. A seguir: caçambas alocadas,
   status de coletas, clientes. **Pré-requisitos no warehouse p/ `fluxo_caixa`:** tabela
   `omie_saldo_conta_corrente` sincronizada e coluna `data_vencimento` nos títulos — se faltarem,
   o bloco correspondente cai como "indisponivel" (degradação graciosa). Atenção: `caixa_hoje`
   reflete o ÚLTIMO saldo sincronizado (`data_saldo_base`), não literalmente hoje — conferir a
   cadência de sync do pipeline no Kondado.
3. ✅ **Parâmetros tipados (v1.6.0)** — `competencia` em `resumo_locanorte`/`faturamento`,
   `ano`/`competencia` em `dre_resultado` (via `_valida_competencia`). ✅ **`dre_resultado` (v1.7.0)** —
   Resultado Operacional Realizado×Projetado, validado contra o doc DAX (+629.043,78 / −780.816,31 / −151.772,53).
   ✅ **`top_clientes` (v1.8.0)** — ranking por contas a receber, com auto-detecção da coluna de cliente.
