...
> **Atualizado em 2026-06-19** para refletir o código realmente no ar (`server.py` v1.11.1,
> 7 tools, financeiro + 1ª tool operacional `coletas`; fix do caixa diário em v1.11.1).
...
Tools (7): status_locanorte, resumo_locanorte, faturamento, fluxo_caixa,
           dre_resultado, top_clientes, coletas (dados reais · contrato v1.11.1)
```
...
## 3. server.py atual (v1.11.1 — no ar)

> **Evolução v1.6.0 → v1.11.1 (resumo):** parâmetros tipados (`competencia`/`ano`/`limite`);
...
> **v1.11.1 — fix do caixa:** `_caixa_hoje` ignora linhas de saldo com `data_saldo > hoje` (a tabela
> de saldo é série diária com dias projetados; antes, o `data_saldo_base` voltava no futuro e o
> "saldo atual" refletia projeção). Expõe `linhas_futuras_ignoradas`.
...
2. ✅ **Tools operacionais/financeiras** — `fluxo_caixa` (v1.5.0), `faturamento` (v1.6.0),
   ...
   ✅ **v1.11.1:** corrigido o `caixa_hoje` que trazia `data_saldo_base` no futuro (ignora saldo de
   dia futuro / `linhas_futuras_ignoradas`).
