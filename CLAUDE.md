...
- Arquivo principal: `server.py` (contrato **v1.11.1**)
...
| `KONDADO_TBL_OS` | não | `omie_servicos_ordens_de_servico` | Ordens de Serviço (`coletas`) |
...
5. **Não rebaixar as tools para placeholder.** O `server.py` já é v1.11.0 com integração Kondado
...
## TOOLS ATUAIS (v1.11.1 — dados reais, retornam dict/JSON) — 7 tools
...
- `fluxo_caixa()` → **(v1.5.0 / fix v1.11.1)** caixa hoje (último saldo por conta **com data ≤ hoje**,
  com `data_saldo_base` e `linhas_futuras_ignoradas`), vencimentos por janela (vencido / 7d / 15d / 30d / 30d+)
  de contas a pagar e a receber EM ABERTO, e projeção curta (7/15/30d). Mesmo `_esta_em_aberto` do resumo
  (`total_aberto` bate com `valor_em_aberto`). Projeção **conservadora**: paga o vencido, não conta recebível
  vencido como entrada. **Fix v1.11.1:** a tabela de saldo é série diária com dias projetados; pegar o maior
  `data_saldo` sem teto trazia `data_saldo_base` no futuro — agora só considera `data_saldo ≤ hoje`.
...
2. ✅ `fluxo_caixa` (v1.5.0), `faturamento` (v1.6.0), `dre_resultado` (v1.7.0), `top_clientes` (v1.8.0)
   e `coletas` (v1.11.0) entregues e validados ao vivo. Próximas tools: clientes por característica/tag,
   detalhe de uma OS específica, ranking por centro de custo (rentabilidade por caminhão).
...
4. ⚠️ **Sync do Kondado**: tabelas operacionais com `last_updated` em 2026-05-26 (cadência/parada do
   pipeline). `coletas` e `caixa_hoje` refletem esse corte — conferir a cadência do sync no Kondado.
   ✅ **Resolvido em v1.11.1:** `caixa_hoje.data_saldo_base` vinha com data futura (a tabela de saldo é
   série diária com dias projetados); o fix passou a ignorar `data_saldo > hoje` (`linhas_futuras_ignoradas`).
